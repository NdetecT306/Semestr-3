...
bool onlyWHERECondition(const string& condition, const Vec& tables, const Vec& currentRows) {
    ...
    if (colInd >= columns.getSize()) return false;
    string leftValue = columns[colInd];
    string rightValue;
    if (rightSide[0] == '\'' && rightSide[rightSide.length()-1] == '\'') {
        ...
        if (rightColInd >= rightCol.getSize()) return false;
        rightValue = rightCol[rightColInd];
    } else {
...
void processCombination(size_t tableIndex, Vec& currentCombination, const Vec& tables, 
                       const Vec& columns, const string& whereCond, Vec& result) {
    if (tableIndex == tables.getSize()) {
        if (checkWhereCondition(whereCond, tables, currentCombination)) {
            ...
            for (size_t colIdx = 0; colIdx < columns.getSize(); colIdx++) {
                ...
                if (colIndex < rowColumns.getSize()) {
                    res_row += rowColumns[colIndex];
                } else {
                    res_row += "NULL";
                }
            }
...
void InsertCommand(const string& input) {
    ...
    bool isOrderTable = (tableName == "order");
    ...
    if (isOrderTable) {
        if (element.getSize() != 6) {
            cerr << "Для таблицы order требуется 6 значений: user_id, pair_id, quantity, price, type, closed" << endl;
            cerr << "Получено значений: " << element.getSize() << endl;
            for (size_t i = 0; i < element.getSize(); i++) {
                cerr << "  [" << i << "]: " << element[i] << endl;
            }
            return;
        }
        // Проверяем тип ордера
        if (element[4] != "sell" && element[4] != "buy") {
            cerr << "Тип ордера должен быть 'sell' или 'buy', получено: " << element[4] << endl;
            return;
        }
    }
    string primaryKeyStr = insertIntoCSV(tableName, element);
    if (primaryKeyStr != "-1" && !primaryKeyStr.empty()) {
        try {
            int primaryKey = stoi(primaryKeyStr);
            string rowData;
            if (tableName == "user") {
                rowData = primaryKeyStr + "," + element[0] + "," + element[1];
            } else if (tableName == "order") {
                rowData = primaryKeyStr + "," + element[0] + "," + element[1] + "," + element[2] + "," + element[3] + "," + element[4] + "," + element[5];
            } else if (tableName == "lot") {
                rowData = primaryKeyStr + "," + element[0];
            } else if (tableName == "pair") {
                rowData = primaryKeyStr + "," + element[0] + "," + element[1];
            } else if (tableName == "user_lot") {
                rowData = element[0] + "," + element[1] + "," + element[2];
                primaryKey = stoi(element[0]) * 1000 + stoi(element[1]);
            } else {
                rowData = primaryKeyStr;
                for (size_t i = 0; i < element.getSize(); i++) {
                    rowData += "," + element[i];
                }
            }
            ...
            cout << "Данные добавлены в таблицу с PK: " << primaryKeyStr << ", но возникли проблемы с SCHEMA_HASH." << endl;
        } catch (const exception& e) {
            cout << "Ошибка преобразования первичного ключа: " << e.what() << endl;
        }
    } else {

...
void loadAllDataToHash() {
    ...
    for (auto it = structure.begin(); it != structure.end(); ++it) {
        ...
        string tablePath = currentSchemaName + "/" + tableName;
        if (!fs::exists(tablePath)) {
            cout << "Директория таблицы '" << tableName << "' не существует: " << tablePath << endl;
            delete[] tableHash;
            continue;
        }
        Vec csvFiles;
        ...
        if (csvFiles.empty()) {
            cout << "Нет CSV файлов для таблицы '" << tableName << "'" << endl;
            delete[] tableHash;
            continue;
        }
...
string insertIntoCSV(const string& tableName, Vec& values) {
    ...
    string primaryKey = "";
    if (needsPrimaryKey) {
        int primaryKeyInt = getNextPrimaryKey(tableName);
